...
line_params = {
    "data_line_width": 0.5,   # Width of actual data lines
    "axis_line_width": 0.8,   # Width of axis spines
    "grid_line_width": 0.5,   # Width of grid lines
    "tick_major_width": 0.8,  # Width of major tick marks
    "tick_minor_width": 0.6,  # Width of minor tick marks
    "markersize" : 3
}
...
def ensure_grid_visibility(ax):
    ...
    # Make all four spines very prominent
...
def configure_axis_style(ax, t_max):
    ...
    # Make all four spines visible and set their width
    for spine_name, spine in ax.spines.items():
    ...
    # Add formatter for y-axis to display only one decimal place
...
def plot_network_dynamics(data, t_max=50, output_file=None):
    ...
    g.set_axis_labels("$Time, t$", "Cooperativity, ⟨a⟩")
